...
function onDeviceReady() {
    ...
    if(deviceType!="BlackBerry" && deviceType!="null") {
        $(".blackberryreminder").hide();
        $(".iosandroidreminder").show();
    } else {
        $(".iosandroidreminder").hide();
        $(".blackberryreminder").show();
    }
    analytics.startTrackerWithId('UA-55304817-1');
}
    ...
    $.ajax({
        ...
        success: function(data) {
           $(data.labeljson).each(function(index, value) {
                ...
                $("#introvideomainid").html('<video id="introvideoid" width="100%" preload="none" controls poster="'+value.introimg+'" onended="wsdcheckfunc()"><source src="'+value.introvideo+'" type="video/mp4"></video>');
                $("#infointrovideomainid").html('<video id="infointrovideoid" width="100%" preload="none" controls poster="'+value.infoimg+'"><source src="'+value.infovideo+'" type="video/mp4"></video>');
                ...
                if(deviceType!="BlackBerry" && deviceType!="null") {
                    $(".publishonfbcls").html('<a href="#" onclick="window.plugins.socialsharing.shareViaFacebook(\''+value.facebooksharingmsg+'\', null, \''+value.appwebsite+'\', function(){console.log(\'share ok\')}, function(msg) {appnotfoundalert(\''+value.facebooknoinstall+'\',\''+value.donebuttontxt+'\',\''+value.poptitle+'\')})" class="langsocfacebookcls">'+value.publishonfbcls+'</a>');
                    $(".twitternowcls").html('<a href="#" onclick="window.plugins.socialsharing.shareViaTwitter(\''+value.twittersharingmsg+'\', null, \''+value.appwebsite+'\', function(){console.log(\'share ok\')}, function(msg) {appnotfoundalert(\''+value.twitternoinstall+'\',\''+value.donebuttontxt+'\',\''+value.poptitle+'\')})" class="langsoctweetcls">'+value.twitternowcls+'</a>');
                ...
                $("#playallvideoid").append('<video class="playallvideoid" id="adultplayallvideo" width="100%" controls preload="none" poster="'+value.adultplayallimg+'" onended="videoEnded()"><source src="'+value.adultplayallvideo+'" type="video/mp4"></video>');
                $("#playallvideoid").append('<video class="playallvideoid" id="youthplayallvideo" width="100%" controls preload="none" poster="'+value.youthplayallimg+'" onended="videoEnded()"><source src="'+value.youthplayallvideo+'" type="video/mp4"></video>');
...
function wsdcheckfunc(){
	nvLangId = sessionStorage.currentLang;
	var nvLangId = Math.floor(nvLangId);
	...
	var nvCDM = nvCM+"/"+nvCD;
	var localData = window.localStorage.getItem(""+deviceType+"wsd_url"+nvLangId+"");
	if(localData==null) {
	    var nvSDM = "";
	    var nvEDM = "";
	} else {
	    var nvSDM = window.localStorage.getItem("startdate");
	    var nvEDM = window.localStorage.getItem("enddate");
	}
...
function wsdservicefunc(nvLangId) {
    doOnOrientationChange();
    var nvLangId = Math.floor(nvLangId);
    var localData = window.localStorage.getItem(""+deviceType+"wsd_url"+nvLangId+"");
	var landscapelocalData = window.localStorage.getItem(""+deviceType+"landscape_wsd_url"+nvLangId+"");
    //var lastupdate = window.localStorage.getItem("lastupdate"+nvLangId+"");
    var nvCurLangId = parseInt(nvLangId) + 1;
    
    $.getJSON(""+wsdutl+"?id="+nvCurLangId, function(result) {
        var wsd_url = result.wsdjson[0].wsd_url;
		var wsd_url_ipad = result.wsdjson[0].wsd_url_ipad;
		var wsd_ipad_landscape = result.wsdjson[0].wsd_ipad_landscape;
		window.localStorage.setItem("startdate", ""+result.wsdjson[0].startdate+"");
		window.localStorage.setItem("enddate", ""+result.wsdjson[0].enddate+"");
		if (wsd_url!="" && wsd_url_ipad!="") {
			if(deviceType=="iPad") {
				downloadFile(wsd_url_ipad,"No");
				downloadFile(wsd_ipad_landscape,"Yes");
				$(".portraitwsdscreen").attr("src",wsd_url_ipad);
				$(".landscapewsdscreen").attr("src",wsd_ipad_landscape);
			} else {
				downloadFile(wsd_url,"No");
				$(".portraitwsdscreen").attr("src",wsd_url);
			}
			//window.localStorage.setItem("lastupdate"+nvLangId+"", ""+result.wsdjson[0].lastupdate+"");
		} else {
			$(".portraitwsdscreen").attr("src",localData);
			$(".landscapewsdscreen").attr("src",landscapelocalData);
		}
    }).fail(function() {
        $(".portraitwsdscreen").attr("src",localData);
		$(".landscapewsdscreen").attr("src",landscapelocalData);
...
function downloadFile(URL,Val) {
    ...
    function fileSystemSuccess(fileSystem) {
        ...
        filetransfer(download_link, fp, nvVal);
    }
...
function filetransfer(download_link, fp, Val) {
    ...
    fileTransfer.download(download_link, fp,
        function(entry) {
            ...
            var nvLangId = Math.floor(nvLangId);
			if(Val=="No") {
				window.localStorage.setItem(""+deviceType+"wsd_url"+nvLangId+"", ""+fp+"");
			} else {
				window.localStorage.setItem(""+deviceType+"landscape_wsd_url"+nvLangId+"", ""+fp+"");
			}
        },
        function(error) {
...
function doOnOrientationChange()
{
    switch(window.orientation)
    {
        case -90:
        case 90:
			$(".portraitwsdscreen").hide();
			$(".landscapewsdscreen").show();
            //alert('landscape');
            break;
        default:
			$(".landscapewsdscreen").hide();
			$(".portraitwsdscreen").show();
            //alert('portrait');
            break;
    }
}

function appnotfoundalert(nvMsg, nvOk, nvTit) {
	navigator.notification.alert(""+nvMsg+"",alertDismissed,""+nvTit+"",""+nvOk+"");
}